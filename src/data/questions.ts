import { Question, DifficultyMode } from '../types';

/**
 * Quiz questions about Baldur's Gate 3 and Forgotten Realms lore.
 * Organized by difficulty mode: Hero (easy), Illithid (medium), Dark Urge (hard).
 */

// ============================================
// HERO MODE - Basic BG3 knowledge
// ============================================
export const heroQuestions: Question[] = [
  {
    question: 'Как зовут вампира-спутника, который может присоединиться к вашей группе?',
    answers: ['Гейл', 'Астарион', 'Уилл', 'Халсин'],
    correct: 1,
    difficulty: 1,
  },
  {
    question: 'Какая раса известна своей связью с Астральным Планом?',
    answers: ['Люди', 'Гитьянки', 'Дроу', 'Полуэльфы'],
    correct: 1,
    difficulty: 1,
  },
  {
    question: 'Как зовут друида-медведя, который может стать компаньоном?',
    answers: ['Минск', 'Халсин', 'Воло', 'Уитерс'],
    correct: 1,
    difficulty: 1,
  },
  {
    question: 'Какой класс у Гейла?',
    answers: ['Чародей', 'Волшебник', 'Колдун', 'Бард'],
    correct: 1,
    difficulty: 1,
  },
  {
    question: 'Как называется город, где происходят события третьего акта?',
    answers: ['Невервинтер', 'Врата Балдура', 'Уотердип', 'Эльтурель'],
    correct: 1,
    difficulty: 2,
  },
  {
    question: 'Кто является лидером друидов в Изумрудной Роще?',
    answers: ['Халсин', 'Кагга', 'Нетти', 'Рат'],
    correct: 0,
    difficulty: 2,
  },
  {
    question: 'Какое существо паразитирует в голове главного героя?',
    answers: ['Интеллект пожиратель', 'Личинка иллитида', 'Церебральный червь', 'Мозговой слизень'],
    correct: 1,
    difficulty: 2,
  },
  {
    question: 'Какой расы Лаэзель?',
    answers: ['Полуорк', 'Гитьянки', 'Тифлинг', 'Дроу'],
    correct: 1,
    difficulty: 2,
  },
  {
    question: 'Кто из компаньонов является колдуном?',
    answers: ['Гейл', 'Астарион', 'Уилл', 'Шэдоухарт'],
    correct: 2,
    difficulty: 3,
  },
  {
    question: 'Как зовут хомяка Минска?',
    answers: ['Бу', 'Пух', 'Шу', 'Ву'],
    correct: 0,
    difficulty: 3,
  },
  {
    question: 'Какой богине служит Шэдоухарт?',
    answers: ['Селунэ', 'Шар', 'Мистра', 'Ллос'],
    correct: 1,
    difficulty: 3,
  },
  {
    question: 'Как называется корабль, на котором начинается игра?',
    answers: ['Наутилоид', 'Астральный Скиф', 'Корабль Гитов', 'Левиафан'],
    correct: 0,
    difficulty: 3,
  },
  {
    question: 'Кто такой Уитерс?',
    answers: ['Торговец', 'Скелет-некромант', 'Хранитель лагеря', 'Всё перечисленное'],
    correct: 2,
    difficulty: 4,
  },
  {
    question: 'Какой титул носит герцог Равенгард?',
    answers: ['Великий герцог', 'Верховный маршал', 'Лорд-протектор', 'Регент'],
    correct: 0,
    difficulty: 4,
  },
  {
    question: 'Кто из персонажей НЕ может стать романтическим партнёром?',
    answers: ['Карлах', 'Минтара', 'Джахейра', 'Минск'],
    correct: 3,
    difficulty: 4,
  },
];

// ============================================
// ILLITHID MODE - Advanced BG3 & D&D lore
// ============================================
export const illithidQuestions: Question[] = [
  {
    question: 'Как называется древняя империя, создавшая Корону Карсуса?',
    answers: ['Калимшан', 'Нетерил', 'Миф Драннор', 'Кормир'],
    correct: 1,
    difficulty: 2,
  },
  {
    question: 'Какая богиня является покровительницей магии в Забытых Королевствах?',
    answers: ['Селунэ', 'Шар', 'Мистра', 'Сьюн'],
    correct: 2,
    difficulty: 2,
  },
  {
    question: 'Кто является лидером культа Абсолюта в начале игры?',
    answers: ['Кетерик Торм', 'Орин Красная', 'Энвер Гортэш', 'Все трое'],
    correct: 3,
    difficulty: 2,
  },
  {
    question: 'Что такое церемофоз?',
    answers: ['Ритуал гитьянки', 'Процесс превращения в иллитида', 'Магия некромантии', 'Тип заклинания'],
    correct: 1,
    difficulty: 3,
  },
  {
    question: 'Кто является богиней тьмы и потери, сестрой Селунэ?',
    answers: ['Ллос', 'Тиамат', 'Шар', 'Талона'],
    correct: 2,
    difficulty: 3,
  },
  {
    question: 'Какой артефакт необходим для уничтожения Абсолюта?',
    answers: ['Нетерильские камни', 'Корона Карсуса', 'Око Бхаала', 'Сфера Аннигиляции'],
    correct: 0,
    difficulty: 3,
  },
  {
    question: 'Как называется демоническая сущность, с которой заключил пакт Уилл?',
    answers: ['Мефистофель', 'Зариэль', 'Мизора', 'Аскодей'],
    correct: 2,
    difficulty: 3,
  },
  {
    question: 'Что случилось с Гейлом до событий игры?',
    answers: ['Был проклят вампиром', 'Поглотил часть Мистры', 'Заключил пакт с дьяволом', 'Стал личом'],
    correct: 1,
    difficulty: 4,
  },
  {
    question: 'Как называется измерение, откуда родом иллитиды?',
    answers: ['Девять Адов', 'Дальний Предел', 'Астральный План', 'Царство Теней'],
    correct: 2,
    difficulty: 4,
  },
  {
    question: 'Кто создал личинок, заражающих героев?',
    answers: ['Абсолют', 'Старший Мозг', 'Влаакит', 'Орфеус'],
    correct: 1,
    difficulty: 4,
  },
  {
    question: 'Какое проклятие лежит на Карлах?',
    answers: ['Вампиризм', 'Инфернальный двигатель в груди', 'Метка Бхаала', 'Проклятие ликантропии'],
    correct: 1,
    difficulty: 4,
  },
  {
    question: 'Кто такой Орфеус?',
    answers: ['Древний дракон', 'Пленённый принц гитьянки', 'Верховный иллитид', 'Аватар Бхаала'],
    correct: 1,
    difficulty: 4,
  },
  {
    question: 'Как называется клинок Лаэзель в начале игры?',
    answers: ['Серебряный меч', 'Астральный клинок', 'Меч гитьянки', 'Пси-клинок'],
    correct: 0,
    difficulty: 5,
  },
  {
    question: 'Что находится в Шрамированных Землях?',
    answers: ['Храм Шар', 'Башня Лунного Восхода', 'Крепость гоблинов', 'Город дроу'],
    correct: 1,
    difficulty: 5,
  },
  {
    question: 'Какая раса правила Фаэруном до эльфов?',
    answers: ['Драконы', 'Сарукх (Создатели)', 'Иллитиды', 'Гиганты'],
    correct: 1,
    difficulty: 5,
  },
];

// ============================================
// DARK URGE MODE - Expert BG3 knowledge, Dark Urge storyline
// ============================================
export const darkUrgeQuestions: Question[] = [
  {
    question: 'Кем был Тёмный Соблазн до потери памяти?',
    answers: ['Жрецом Бхаала', 'Избранным Бхаала', 'Аватаром Бхаала', 'Слугой Мёртвой Тройки'],
    correct: 1,
    difficulty: 3,
  },
  {
    question: 'Кто является отцом Тёмного Соблазна?',
    answers: ['Горион', 'Саревок', 'Бхаал', 'Энвер Гортэш'],
    correct: 2,
    difficulty: 3,
  },
  {
    question: 'Как зовут дворецкого Тёмного Соблазна?',
    answers: ['Скелларак', 'Скраг', 'Скелерио', 'Склонн'],
    correct: 2,
    difficulty: 3,
  },
  {
    question: 'Что делала Орин с Тёмным Соблазном?',
    answers: ['Служила ему', 'Предала и украла его место', 'Была его наставником', 'Не знала о нём'],
    correct: 1,
    difficulty: 4,
  },
  {
    question: 'Кем Орин приходится Тёмному Соблазну?',
    answers: ['Сестра', 'Дочь', 'Мать', 'Не родственники'],
    correct: 0,
    difficulty: 4,
  },
  {
    question: 'Где находится Храм Бхаала?',
    answers: ['Под Вратами Балдура', 'В Царстве Теней', 'В Подземье', 'В Девяти Адах'],
    correct: 0,
    difficulty: 4,
  },
  {
    question: 'Какой титул носит Тёмный Соблазн среди культа?',
    answers: ['Великий Палач', 'Избранный', 'Отпрыск', 'Чемпион Бхаала'],
    correct: 0,
    difficulty: 4,
  },
  {
    question: 'Что символизируют руки Бхаала?',
    answers: ['Милосердие и гнев', 'Убийство и воскрешение', 'Кровь и кости', 'Смерть и возрождение'],
    correct: 1,
    difficulty: 4,
  },
  {
    question: 'Кто был первой жертвой, которую Тёмный Соблазн убил в прошлом?',
    answers: ['Невинный путник', 'Его собственная мать', 'Культист Бхаала', 'Это неизвестно'],
    correct: 1,
    difficulty: 5,
  },
  {
    question: 'Как называется особая форма Орин?',
    answers: ['Терзающая', 'Резня', 'Нетленная', 'Оскверняющая'],
    correct: 1,
    difficulty: 5,
  },
  {
    question: 'Что нужно сделать, чтобы победить Бхаала как Тёмный Соблазн?',
    answers: ['Убить Орин', 'Отвергнуть его дар', 'Принести себя в жертву', 'Стать новым богом убийств'],
    correct: 1,
    difficulty: 5,
  },
  {
    question: 'Какое заклинание получает Тёмный Соблазн от Бхаала?',
    answers: ['Власть над смертью', 'Убийственный инстинкт', 'Кровавое безумие', 'Неудержимое убийство'],
    correct: 3,
    difficulty: 5,
  },
  {
    question: 'Что происходит с компаньоном, если Тёмный Соблазн не сопротивляется побуждениям?',
    answers: ['Компаньон уходит', 'Компаньон умирает от рук Соблазна', 'Ничего', 'Компаньон предаёт'],
    correct: 1,
    difficulty: 5,
  },
  {
    question: 'Кто может полностью исцелить Тёмный Соблазн от проклятия Бхаала?',
    answers: ['Уитерс', 'Шэдоухарт', 'Никто', 'Любой жрец'],
    correct: 2,
    difficulty: 5,
  },
  {
    question: 'Как называется секретная концовка для Тёмного Соблазна?',
    answers: ['Божественное Вознесение', 'Абсолютная Власть', 'Бхаалово Наследие', 'Повелитель Убийств'],
    correct: 3,
    difficulty: 5,
  },
];

/**
 * Get questions for a specific difficulty mode.
 */
export const getQuestionsForMode = (mode: DifficultyMode): Question[] => {
  switch (mode) {
    case 'hero':
      return heroQuestions;
    case 'illithid':
      return illithidQuestions;
    case 'darkUrge':
      return darkUrgeQuestions;
    default:
      return heroQuestions;
  }
};

/**
 * Prize ladder in gold pieces.
 */
export const prizes: string[] = [
  '500',
  '1,000',
  '2,000',
  '3,000',
  '5,000',
  '10,000',
  '15,000',
  '25,000',
  '50,000',
  '100,000',
  '200,000',
  '400,000',
  '800,000',
  '1,500,000',
  '3,000,000',
];

/**
 * Indices of guaranteed prize levels (safety nets).
 */
export const guaranteedPrizes: number[] = [4, 9, 14];

/**
 * Companion names for "Phone a Friend" lifeline.
 */
export const companionNames: string[] = [
  'Астарион',
  'Гейл',
  'Шэдоухарт',
  'Лаэзель',
  'Халсин',
];
